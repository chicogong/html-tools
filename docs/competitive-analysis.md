...
1. 选 ~20 个高搜索量工具（JSON 格式化、时间戳、二维码、Base64、正则测试等）集中投入。
2. **为工具页补 `FAQPage` 结构化数据 + 可见 FAQ 区块** —— 命中长尾词与精选摘要。
   - ✅ 已落地参考实现：`tools/dev/json-formatter.html`（popularity 100，置顶工具）。
   - ⬜ 待按此模式推广到其余高 popularity 工具。
3. 分类页（`tools/<cat>/index.html`）做成内容枢纽，加品类导购文案。
4. 用 Search Console 盯「有曝光无点击」的工具页，针对性优化标题 / 描述。
5. 持续维护 `llms.txt`，保住 AI 搜索引用这一增量入口。
