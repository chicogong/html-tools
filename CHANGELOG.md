...
本版本将 WebUtils 从“大量工具页面”收口为可持续维护的本地优先工具集，强化独立导出、隐私边界、浏览器验收和开源贡献入口。

### 新增

- 首页“最近使用”工作流，与搜索、分类、收藏和 URL 状态联动
- 已登记工具的 standalone HTML 导出能力，自动内联仓库自有的共享 CSS/JavaScript
- Playwright Browser Smoke 基线，覆盖首页发现、收藏、最近使用、移动端溢出与 5 个代表工具流程
- 英文项目概览、中英文切换入口与可自动同步的工具数量

### 改进

- 工具规模更新为 1088 个，补齐工具页唯一 H1 及对应结构门禁
- 收紧本地处理、浏览器权限、外部 API/CDN 和离线能力的文档表述，避免泛化的“纯前端即绝对离线”承诺
- JSON 格式化、时间戳转换、URL 编解码与 Base64 工具的键盘、动态状态、Unicode 和文件处理可用性
- Issue/PR 模板与贡献检查清单，补充复现环境、敏感信息与网络边界要求

### 工程与发布

- 单测基线扩展至 68 项，共享工具样式纳入 Stylelint，部署 Actions 升级至 Node.js 24 运行时
- 发布工作流增加版本一致性、历史泄漏、个人路径、许可证、可重现归档与当前 `master` 标签校验
- 部署包与发布归档均包含中英文 README、LICENSE 和 NOTICE，并验证与源文件一致

### 修复

- Base64 Unicode 编解码、文件导入导出与 UTF-8 BOM 保留
- URL 编解码错误恢复、时间戳输入与 JSON 结果动态通知
- 移动端搜索键盘、分类阴影裁剪、首页溢出与多项工具运行时异常
